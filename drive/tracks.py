...
def _big_red():
    ...
    the track spends the rest of itself going *down* - about 190 units of it
    across 2300, so the finish sits nearly two loops' height below the start and
    the one real climb on the whole track is the loop itself, the moment it
    hauls you back up rather than a trick sitting in the middle of a flat road.

    Three moments are genuinely airborne rather than just steep. Out of the
    slow hairpin a pad feeds a long, shallow kicker into a gap wide enough
    that the car is in the air for the best part of two seconds, dropping
    onto a stretch of road well under where it left - the one place on the
    track where "down" is something you fly rather than corner your way
    through. A second, smaller gap later on is the same idea in miniature, and
    a third, off the last pad and the same size as the first, closes the lap
    the way it opened: in the air. All three are pure ballistics: ``gap``'s
    own bow is too gentle for a fall this size and reads as a kink the lap
    model brakes hard for, so each is authored with an explicit ``bow`` that
    matches the kicker's exit angle and lets the model see the same
    rising-then-falling arc the car actually flies.

    Neither big jump is as long as it could be made to look on paper.
    ...
    distance or drop. A shallow kicker (a 4-unit rise over 50, on both) buys
    most of that budget back: pitch grows with time in the air, not with how
    far it falls, so a launch angle close to flat gets a deep drop and a long
    carry for the same rotation a steeper, shorter kicker would spend on
    ...
    can lean on is what carries speed rather than scrubbing it; the one tight
    hairpin exists so the big ones have something to be big *against*. It is
    in ``EXPOSED`` and keeps barriers in only four places - the loop, and the
    landing straight after each of the three jumps, all of them being where a
    car arrives with no chance to correct rather than an ordinary corner exit.
    Everywhere else the edge is just the edge: a downhill track built around
    how far down it is loses the point of itself walled all the way round.

    The four boost pads are the only ones in the pool. They are all on
    straights and all with a long run to the next braking point - out of the
    slow corner and into the first jump, onto the loop, off the hairpin and
    into the last - because a pad is worth about a second of unarguable speed
    and the place for that is somewhere you can use it, never into a corner
    where all it does is take the corner away.
    """
