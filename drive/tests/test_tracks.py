...
import tuning as T
from conftest import NO_CUT_MEDALS_YET
...
def test_gold_is_cut_from_the_board_and_not_from_the_estimate(track):
    ...
    """
    if track["slug"] in NO_CUT_MEDALS_YET:
        # A track nobody has driven has no times to cut from, so it uses the
        # derivation this test exists to forbid - see `NO_CUT_MEDALS_YET`. Asserted
        # the other way so a stale entry cannot hide a real fallback.
        assert not track["medal_times"], (
            "%s has cut medal times now - drop it from NO_CUT_MEDALS_YET in "
