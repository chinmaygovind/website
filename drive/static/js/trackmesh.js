...
// Palette
// ---------------------------------------------------------------------------
// **A palette arrives on the track, from Python.** There used to be a `PALETTES`
// object of fifteen entries here, 560 lines of it, and it was the seam that made
// this file hard to work in: a track's geometry was authored in tracks.py and its
// appearance in this file, so anything that was *both* had to be written down
// twice in two languages. Sandy Cove's waterline and the Costco shell are exactly
// that - the road is authored against them - and they were held together by tests
// that scraped this file with a regular expression.
//
// They are `tracks/<slug>/palette.py` now, attached to the track dict by the
// loader, and they reach here through plumbing that already existed:
// `_track_payload` inlines the whole track as `window.DRIVE_TRACK`, and `jsrt`
// hands the pool to QuickJS as JSON. Colours are still packed RGB integers, so
// nothing downstream of this function changed at all.
//
// The fallback is for the anti-cheat and the tests, which build tracks from
// hand-made fixtures rather than from the loader. A real track always has `pal`;
// `tracks/look.py` refuses to load one that does not.
const FALLBACK_PAL = {
  road: 0x4d5464, kerb: 0xf4f4f2, kerb2: 0xe8453c, ground: 0x5ea364,
  rail: 0xf2eee8, prop: 0x37624a, deco: 0xf2c94c, fog: 0xc9d3dc, density: 0,
};

export function palette(track) {
  return track.pal || FALLBACK_PAL;
}

// ---------------------------------------------------------------------------
// Per-track scenery
// ---------------------------------------------------------------------------
// Some tracks need mesh code nothing else does. The Costco is a shell the road
// drives *into*, with racking you can hit and a roof over your head; no amount of
// palette configuration produces that, and no other track wants it.
//
// So a track may ship `tracks/<slug>/scenery.js`, which registers itself here:
//
//     (function () {
...
export function buildTrack(track, T) {
  ...
  //
  // The context is everything a scenery function could otherwise only get by
  // being defined inside `buildTrack`: the three mesh buffers, the collider
  // (because a shelf you can drive through is not a shelf), the `signs` list
