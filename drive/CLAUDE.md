...
templates (`play.html`) blank it - a fixed bar over a
...
Drive's manifest is `display: fullscreen`, so an installed Drive
matches `fullscreen` and never `standalone`. `navigator.standalone` covers iOS, which
